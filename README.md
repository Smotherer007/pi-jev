...
### One honest caveat about the API

TypeSafe documents the *request* shape. The *response* shape is described in
prose and blog samples, so field names such as `usage.inputTokens` versus
`usage.input_tokens` are inferred rather than confirmed against a live account.

The parser accepts every plausible spelling, and when it cannot read a
probability it records the answer as `degraded` with `p=0.5` and keeps the raw
body rather than inventing a confidence. Nothing downstream depends on a guess.

**On your first real call, run `/jev` and check the token counts.** If they read
zero, one field name needs adding to `pickNumber` in `src/questions.ts`. That is
...
```bash
npm install
npm test        # 239 tests, no network required
npm run typecheck
```
